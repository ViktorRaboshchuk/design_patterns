"""
Паттерн Адаптер описывает технику адаптации интерфейса.
Задача состоит в том, чтобы один класс мог воспользоваться другим - с несовместным интерфейсом -
без внесения каких либо  изменений в оба класса
...
if sys.version_info[:2] >= (3, 3):
    class Renderer(metaclass=abc.ABCMeta):
        """
        Этот метод используется встроенной функцией isinstance(),
        чтобы определить, является ли объект, переданный ей в качестве первого аргумента,
        подклассом класса(или любого из классов перечисленных в кортеже),
        переданного во втором аргументе
        """
        @classmethod
        def __subclasshook__(cls, sub_cls):
            if cls is Renderer:
                attributes = collections.ChainMap(*(Superclass.__dict__
                        for Superclass in sub_cls.__mro__))
